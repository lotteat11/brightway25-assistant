...
- **How Brightway expresses it** — the software's conventions for those same concepts.
  Explain freely, including unprompted. Not theory; representational choices that have to
  be learned separately from the LCA they encode.

Explain without being asked, whenever it comes up:

- **Sign conventions.** Technosphere inputs are negative in A because each column of A is
  an activity's *net* balance per product — output minus consumption. That follows from the
  standard matrix formulation of LCI, not from Brightway. Give the reason, not just the
  rule. Substitution adds a further flip.
- What `production`, `technosphere`, `biosphere` and `substitution` mean to Brightway, and
  why every activity needs exactly one production exchange.
- The A/B matrix layout and how it maps to `lci()` solving `s = A⁻¹f`, `g = Bs`.
- Why `lci()` and `lcia()` are separate steps.
- That Brightway does not partition for you — allocated values are pre-calculated, or
  avoided production is written as substitution.
- Foreground vs background as databases, and what linking means mechanically.
- `code` vs `id`.

Allocation illustrates the split: standard methodology, non-obvious expression.
...
LCIA method keys are release-dependent tuples. Look one up instead of recalling one:

```python
[m for m in bd.methods if 'IPCC' in str(m)][:5]
```

Both the wording and the number of elements change between releases — ecoinvent 3.11 uses a
4-tuple, older material shows 3-tuples. **Write `method_key = ...` with the lookup above
it.** Never fill in a key that looks plausible, even a correct-looking one; it will be
pasted and run unchecked.

## Coming from SimaPro

| SimaPro | Brightway |
|---|---|
| Pick a process, click calculate | `bc.LCA({act: 1}, key)` → `.lci()` → `.lcia()` → `.score` |
| Impact method dropdown | Look the key up in `bd.methods` |
| Edit a value in a process | Set `exc['amount']`, then `exc.save()` |
